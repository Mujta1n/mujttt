...
# Función para descifrar archivos MIJ
...
# Función para filtrar contenido MIJ
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 2:
        print("Usage: python decrypt_mij.py <encrypted_mij_file>")
        sys.exit(1)

    encrypted_mij_file = sys.argv[1]

    with open(encrypted_mij_file, 'rb') as f:
        encrypted_mij = f.read()

    decrypted_content = decrypt_mij_file(encrypted_mij, DECRYPTION_KEY)
    if decrypted_content:
        filtered_content = filter_mij_content(decrypted_content)

...
# Función para descifrar archivos MRC
...
# Función para filtrar contenido MRC
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 2:
        print("Usage: python decrypt_mrc.py <encrypted_mrc_file>")
        sys.exit(1)

    encrypted_mrc_file = sys.argv[1]

    with open(encrypted_mrc_file, 'rb') as f:
        encrypted_mrc = f.read()

    decrypted_content = decrypt_mrc_file(encrypted_mrc, DECRYPTION_KEY)
    if decrypted_content:
        filtered_content = filter_mrc_content(decrypted_content)
